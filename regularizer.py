...
class DQN:
    def __init__(self, num_states, num_actions, env, scope, fisher_multiplier, penalty, task_specific=False, num_hidden=24, num_hidden_2=12,
    ...
    def create_model(self):
        ...
        with tf.variable_scope(self.scope):
            ...
            self.W_1 = tf.Variable(
                tf.random_normal([self.num_states, self.num_hidden], stddev=1.0 / np.sqrt(self.num_states)), name="W_1")
            ...
            if self.task_specific and self.mod_all_layers:
                self.h_1 = tf.multiply(
                    tf.nn.relu(tf.matmul(self.inputs, self.W_1) + tf.matmul(self.task_num_oh, self.b_1)),
                    tf.matmul(self.task_num_oh, self.g_1), name="h_1")
            else:
                self.h_1 = tf.nn.relu(tf.matmul(self.inputs, self.W_1) + self.b_1, name="h_1")

            # Second hidden layer
            self.W_2 = tf.Variable(
                tf.random_normal([self.num_hidden, self.num_hidden_2], stddev=1.0 / np.sqrt(self.num_hidden)),
                name="W_2")
            if self.task_specific and self.mod_all_layers:
            ...
            if self.task_specific and self.mod_all_layers:
                self.h_2 = tf.multiply(
                    tf.nn.relu(tf.matmul(self.h_1, self.W_2) + tf.matmul(self.task_num_oh, self.b_2)),
                    tf.matmul(self.task_num_oh, self.g_2), name="h_2")
            else:
                self.h_2 = tf.nn.relu(tf.matmul(self.h_1, self.W_2) + self.b_2, name="h_2")

            # Output layer
            self.W_3 = tf.Variable(
                tf.random_normal([self.num_hidden_2, self.num_actions], stddev=1.0 / np.sqrt(self.num_hidden_2)),
                name="W_3")
            if self.task_specific:
                ...
                self.Qout = tf.multiply((tf.matmul(self.h_2, self.W_3) + tf.matmul(self.task_num_oh, self.b_3)),
                                        tf.matmul(self.task_num_oh, self.g_3))
            else:
                if self.final_bias:
            ...
            if self.update_summary:
                self.prev_weights = [tf.placeholder(shape=var.get_shape(), dtype=var.dtype) for var in
                                     tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope=self.scope)]
                self.diff_mags = [tf.log(tf.abs(curr - prev) + TINY_CONSTANT, name=curr.op.name + '/diffmag') for
                                  curr, prev in
                                  zip(tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope=self.scope),
                                      self.prev_weights)]
            ...
            if self.smax_tau or self.soft_tau:
                self.action_grads = [tf.squeeze(tf.reduce_mean(abs_grad, axis=-1)) for abs_grad in [
                    tf.stack([tf.abs(tf.gradients(a_prob, var)) for a_prob in tf.unstack(self.action_probs, axis=1)],
                             axis=-1) for var in tf.get_collection("metaplastic", scope=self.scope)]]
        ...
        if self.task_specific and self.ts_flow_pause and self.metaplastic:
            ...
            for var in tf.get_collection("metaplastic", scope=self.scope):
                if var.name in ts_var_names:
                    pause_flow_mod = tf.multiply(tf.reshape(self.task_num_oh, (self.num_tasks, 1)),
                                                 tf.ones(var.get_shape()))
                else:
    ...
    def act(self, state, curr_task, test=False):
        ...
        if not test and (np.random.rand(1) < self.epsilon):
            ...
            if self.soft_tau or self.smax_tau:
                action_probs, self.value = sess.run([self.action_probs, self.Vout],
                                                    feed_dict={self.inputs: state, self.task_number: [task_num]})
        ...
        for state, action, reward, next_state, done, task_num in minibatch:
            ...
            if not done:
                if target_agent == None:
                    ...
                    if self.soft_tau:
                        V1 = sess.run(target_agent.Vout,
                                      feed_dict={target_agent.inputs: next_state, target_agent.task_number: [task_num]})
                    else:
                        Q1 = sess.run(target_agent.Qout,
                                      feed_dict={target_agent.inputs: next_state, target_agent.task_number: [task_num]})
                        if self.double:
                            best_a = sess.run(self.predict,
                                              feed_dict={self.inputs: next_state, self.task_number: [task_num]})
                            V1 = Q1[0][best_a]
                        else:
                            V1 = np.max(Q1)
            ...
            if target_agent == None:
                ...
                targetQ = sess.run(target_agent.Qout,
                                   feed_dict={target_agent.inputs: state, target_agent.task_number: [task_num]})

            targetQ[0][action] = target
            if self.mod_flow:
                loss, flow_mod, _ = sess.run([self.loss, self.action_grads, self.updateModel],
                                             feed_dict={self.inputs: state, self.task_number: [task_num],
                                                        self.nextQ: targetQ, self.lr_placeholder: self.learning_rate})
            else:
                loss, _ = sess.run([self.loss, self.updateModel],
                                   feed_dict={self.inputs: state, self.task_number: [task_num], self.nextQ: targetQ,
    ...
    def replay_meta(self, batch_size, target_agent=None, curr_task=None):
        sess = tf.get_default_session()
        if self.multitask:
            if curr_task == -1:
                minibatch = []
        ...
        # init weights
        init_weights_values = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='policy')
        ...
        for state, action, reward, next_state, done, task_num in minibatch:
            ...
            loss, _ = sess.run([self.loss, self.updateModel],
                               feed_dict={self.inputs: state, self.task_number: [task_num], self.nextQ: targetQ,
                                          self.lr_placeholder: 0.0001})

            losses.append(loss)

        # weights update with \beta learning rate
        batch_weights_values = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='policy')

        batch_weights_update = []
        for i in range(len(batch_weights_values)):
            batch_weights_update.append(init_weights_values[i] - batch_weights_values[i])

        target_vars = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='target')
        ops = []
        for i in range(len(target_vars)):
            ops.append(target_vars[i].assign(batch_weights_update[i]))
